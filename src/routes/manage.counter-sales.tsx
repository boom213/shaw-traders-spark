...
import { AlertTriangle, Banknote, Download, Minus, PackagePlus, Plus, QrCode, ReceiptText, RotateCcw, Search, UserRound } from "lucide-react";
...
function CounterSalesPage() {
  ...
  const selectedPaymentVendor = setup?.vendors.find((vendor) => vendor.id === payment.vendorId);
  ...
  const paymentMutation = useMutation({
    mutationFn: () => recordCounterPayment({ data: { orderId: paymentSale!.orderId, amount: Number(payment.amount), method: payment.method, reference: payment.reference, note: payment.note, receivedOn: payment.receivedOn, vendorId: payment.vendorId } }),
    onSuccess: async (result) => { if (!result.ok) return toast.error(result.error); toast.success(`Payment recorded · ${money(result.balance)} remaining`); setPaymentSale(null); setPayment({ amount: "", method: "Cash", reference: "", note: "", receivedOn: today(), vendorId: "" }); await refresh(); },
    onError: (error) => toast.error(error.message),
  });
  ...
  return (
    <section className="space-y-6">
      ...
      <AlertDialog open={Boolean(paymentSale)} onOpenChange={(open) => { if (!open) { setPaymentSale(null); setPayment((value) => ({ ...value, vendorId: "" })); } }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Record offline payment</AlertDialogTitle><AlertDialogDescription>{paymentSale?.humanId} · balance {money(paymentSale?.balance ?? 0)}</AlertDialogDescription></AlertDialogHeader><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-medium">Amount<Input type="number" min="0.01" max={paymentSale?.balance} step="0.01" value={payment.amount} onChange={(event) => setPayment({ ...payment, amount: event.target.value })} /></label><label className="text-sm font-medium">Received on<Input type="date" value={payment.receivedOn} onChange={(event) => setPayment({ ...payment, receivedOn: event.target.value })} /></label><label className="text-sm font-medium">Method<select className="mt-1 h-10 w-full rounded-md border bg-background px-3" value={payment.method} onChange={(event) => setPayment({ ...payment, method: event.target.value, vendorId: event.target.value === "Vendor QR" ? payment.vendorId : "" })}><option>Cash</option><option>UPI</option><option>Vendor QR</option><option>Bank transfer</option><option>Cheque</option><option>Other</option></select></label><label className="text-sm font-medium">Reference<Input value={payment.reference} onChange={(event) => setPayment({ ...payment, reference: event.target.value })} placeholder={["UPI", "Vendor QR", "Bank transfer"].includes(payment.method) ? "Required UTR / reference" : "Optional reference"} /></label>{payment.method === "Vendor QR" && <><label className="text-sm font-medium sm:col-span-2">Vendor QR <span className="font-normal text-muted-foreground">(optional)</span><select aria-label="Vendor QR code" className="mt-1 h-10 w-full rounded-md border bg-background px-3" value={payment.vendorId} onChange={(event) => setPayment({ ...payment, vendorId: event.target.value })}><option value="">No QR selected</option>{setup?.vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}</select></label>{selectedPaymentVendor && <div className="flex items-center gap-4 rounded-md border bg-muted/40 p-3 sm:col-span-2">{selectedPaymentVendor.qrUrl ? <img src={selectedPaymentVendor.qrUrl} alt={`${selectedPaymentVendor.name} payment QR code`} className="size-36 rounded-md border bg-background object-contain" /> : <div className="grid size-36 place-items-center rounded-md border bg-background"><QrCode className="size-9 text-muted-foreground" /></div>}<div><p className="font-semibold">{selectedPaymentVendor.name}</p>{selectedPaymentVendor.upiId && <p className="mt-1 text-sm text-muted-foreground">UPI: {selectedPaymentVendor.upiId}</p>}<p className="mt-2 text-xs text-muted-foreground">Selecting this vendor records the payment in Vendor Payments.</p></div></div>}</>}<label className="text-sm font-medium sm:col-span-2">Note<Input value={payment.note} onChange={(event) => setPayment({ ...payment, note: event.target.value })} placeholder="Optional internal note" /></label></div><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction disabled={paymentMutation.isPending || Number(payment.amount) <= 0 || Number(payment.amount) > (paymentSale?.balance ?? 0) || (["UPI", "Vendor QR", "Bank transfer"].includes(payment.method) && !payment.reference.trim())} onClick={(event) => { event.preventDefault(); paymentMutation.mutate(); }}>{paymentMutation.isPending ? "Recording…" : "Record payment"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
