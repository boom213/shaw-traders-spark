...
function CounterSalesPage() {
  ...
  return (
    <section className="space-y-6">
      ...
      <AlertDialog open={Boolean(selectedSale)} onOpenChange={(open) => !open && setSelectedSale(null)}><AlertDialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><AlertDialogHeader><AlertDialogTitle>{selectedSale?.humanId} · {selectedSale?.customerName}</AlertDialogTitle><AlertDialogDescription>{selectedSale?.invoiceKind === "gst" ? "GST tax invoice" : "Non-GST bill"} created by {selectedSale?.createdBy}</AlertDialogDescription></AlertDialogHeader>{selectedSale && <SaleDetails sale={selectedSale} />}<AlertDialogFooter><AlertDialogCancel>Close</AlertDialogCancel><Button onClick={() => downloadInvoice(selectedSale!)}><Download className="size-4" /> Invoice PDF</Button></AlertDialogFooter></AlertDialogContent></AlertDialog>
