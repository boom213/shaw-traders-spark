    ...
    <div className="space-y-8">
      ...
      <section className="space-y-4 border-t border-border pt-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <Field label="Account type" htmlFor="customer-type"><Select disabled={!canEdit} value={account.customerType} onValueChange={(value: "retail" | "trade") => setAccount((current) => ({ ...current, customerType: value }))}><SelectTrigger id="customer-type"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="retail">Retail</SelectItem><SelectItem value="trade">Trade</SelectItem></SelectContent></Select></Field>
          <Field label="Price tier" htmlFor="price-tier"><Select disabled={!canEdit} value={account.priceTier} onValueChange={(value: "retail" | "trade" | "distributor") => setAccount((current) => ({ ...current, priceTier: value }))}><SelectTrigger id="price-tier"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="retail">Retail</SelectItem><SelectItem value="trade">Trade</SelectItem><SelectItem value="distributor">Distributor</SelectItem></SelectContent></Select></Field>
          <Field label="Credit limit" htmlFor="credit-limit"><Input id="credit-limit" disabled={!canEdit} type="number" min="0" max="100000000" value={account.creditLimit} onChange={(event) => setAccount((current) => ({ ...current, creditLimit: event.target.value }))} /></Field>
          <Field label="Payment terms (days)" htmlFor="payment-terms"><Input id="payment-terms" disabled={!canEdit} type="number" min="0" max="365" value={account.paymentTermsDays} onChange={(event) => setAccount((current) => ({ ...current, paymentTermsDays: event.target.value }))} /></Field>
        </div>
      ...
      <section className="space-y-4 border-t border-border pt-6">
        ...
        {canEdit && (
          <div className="space-y-4 rounded-lg border border-border bg-surface p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2"><Field label="Address" htmlFor="address-line"><Input id="address-line" value={address.line1} onChange={(event) => setAddress((current) => ({ ...current, line1: event.target.value }))} /></Field><Field label="Landmark" htmlFor="address-landmark"><Input id="address-landmark" value={address.landmark} onChange={(event) => setAddress((current) => ({ ...current, landmark: event.target.value }))} /></Field><Field label="City" htmlFor="address-city"><Input id="address-city" value={address.city} onChange={(event) => setAddress((current) => ({ ...current, city: event.target.value }))} /></Field><Field label="State" htmlFor="address-state"><Input id="address-state" value={address.state} onChange={(event) => setAddress((current) => ({ ...current, state: event.target.value }))} /></Field><Field label="Pincode" htmlFor="address-pincode"><Input id="address-pincode" inputMode="numeric" maxLength={6} value={address.pincode} onChange={(event) => setAddress((current) => ({ ...current, pincode: event.target.value.replace(/\D/g, "").slice(0, 6) }))} /></Field><label className="flex items-center gap-2 self-end pb-2 text-sm font-medium"><Checkbox checked={address.isDefault} onCheckedChange={(checked) => setAddress((current) => ({ ...current, isDefault: checked === true }))} /> Default address</label></div>
